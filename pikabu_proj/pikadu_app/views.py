...
def enter_name(request):
    if request.method == 'POST':
        form = CreatePostForm(request.POST)
        if form.is_valid():
            Post(title=request.POST['title'], body=request.POST['body']).save()
            return redirect('post_list')
        return redirect('enter_name_url')
    else:
        form = CreatePostForm()
        return render(request, 'pikadu_app/enter_name.html', context={'title':'Ввод имени',
                                                                      'form':form})
